...
from granuscore.granularity_predictor import HitWithoutDis0Predictor
...
def compute_hierarchy_features(
    ...
    index_scores = lgb_features[:, :k]  # shape (b, k)
    dists = lgb_features[:, k:2 * k]    # shape (b, k)
...
# =========================
def main():
    scorer = HitWithoutDis0Predictor(
    ...
    dump_pickle(val_lgb, f"{PROJECT_DIR}/data/precalc_data/{INDEX_NAME}-embedding-{search_method}-eval_data_lgb.pkl")

    enrich_with_hierarchy_features(
        test_lgb, scorer, BATCH_SIZE, k
    )
    dump_pickle(test_lgb, f"{PROJECT_DIR}/data/precalc_data/{INDEX_NAME}-embedding-{search_method}-test_data_lgb.pkl")

    enrich_with_hierarchy_features(
        train_lgb, scorer, BATCH_SIZE, k
    )
    dump_pickle(train_lgb, f"{PROJECT_DIR}/data/precalc_data/{INDEX_NAME}-embedding-{search_method}-train_data_lgb.pkl")
